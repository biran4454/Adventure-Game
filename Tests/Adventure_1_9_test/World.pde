public class World{
  ...
  public boolean moveCheck(int bX, int bY){
    ...
    if(blockB == null){
      if(blockA.destroyed){
        ...
        if(! blockA.destroyed){
          snap(player.x, player.y, player.bX, player.bY);
  ...
  public void move(int x, int y){
    posX -= x;
    posY -= y;
    posBX = 0 - floor(posX / blockSize);
    posBY = 0 - floor(posY / blockSize);
    player.movePlayer(x, y, blockSize);
  }
  
  private void snap(int x, int y, int bx, int by){
    println(x);
    println(bx);
    println(bx * blockSize);
    println(x < bx * blockSize);
    if(x < (bx + 1) * blockSize){
      println(x);
      println(bx);
      println(blockSize);
      println("Moving back " + str(((bx + 1) * blockSize) - x));
      //move((((bx + 1) * blockSize) - x), 0);
    }
  }
}
